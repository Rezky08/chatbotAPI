...
@section('main')
                    ...
                    <div class="dropdown-menu">
                        <div class="dropdown-content">
                            ...
                            <a href="{{ url(URL::current() . '/add') }}" class="dropdown-item">
                                <span>Add Some Answer</span>
                                <span class="icon has-text-primary"><i class="fa fa-exclamation"></i></span>
                            </a>
                        </div>
                    </div>
                </div>
    ...
    <div class="box">
            ...
            <table class="table is-fullwidth">
                ...
                <tbody>
                    @foreach ($answers as $key => $answer)
                        <tr>
                            <td>{{ $number++ }}</td>
                            <td>{{ $answer->application->client->name }}</td>
                            <td>{{ $answer->text }}</td>
                            <td>
                                @if ($answer->label)
                            ...
                            <td>
                                <div class="columns is-vcentered">
                                    <div class="column">
                                        <form action="{{ url('answer/' . $answer->application->id . '/' . $answer->id) }}"
                                            method="POST">
                                            @csrf
                                            @method('DELETE')
                                    ...
                                    <div class="column">
                                        <a href="{{ url(URL::current() . '/' . $answer->id . '/edit') }}">
                                            <span class="icon has-text-info has-tooltip-left" data-tooltip="Edit Answer">
        ...
        @if ($answers)
            <x-pagination :paginator="$answers"></x-pagination>
        @endif
                    ...
                    <form action="{{ url(URL::current() . '/bulk', []) }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="file has-name is-fullwidth">
                            <label class="file-label">
                                <input type="file" class="file-input" name="answer_file" id="answer_file">
                                <span class="file-cta">
                                    <span class="file-icon">
                                        <i class="fa fa-upload"></i>
                                    </span>
                                    <span class="file-label">Choose a answer file...</span>
                                </span>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('select.select2').on('change', function() {
                let base = "{{ url('') }}";
                window.location = base + "/answer/" + $(this).val();
                console.log($(this).val());
            });
            $("#answer_file").on('change', function() {
                let filename = getFileName($('#answer_file').val());
